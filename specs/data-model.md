...
| storagePath | VARCHAR(500) | Yes | Opaque handle generated by the active `StorageProvider` (§6.10) — under `PostgresBlobStorageProvider`, a generated key with no filesystem meaning of its own; kept so a future object-storage-backed provider can repurpose it as a real object key without a schema change | Unique | — |
| fileBytes | BYTEA / Bytes | Yes | The actual file content | ≤ 10,485,760 (10MB, same bound as `fileSizeBytes`) | — |
...
- **`fileBytes` added to close spec-review.md SR-002**: `technical-architecture.md` §9/TA-1 mandates
  storing attachment content directly in Postgres, since Vercel's serverless filesystem cannot
  durably persist an uploaded file across invocations — this column is where that content actually
  lives. `storagePath` is retained as the `StorageProvider` interface's opaque handle (what `save`
  returns and `retrieve`/`delete` accept), not as a second copy of the content or a real filesystem
  reference.
- A simulated attachment's `fileBytes` resolves to one shared, bundled placeholder file's bytes per
  accepted MIME type (§6.10.2), never to real content — `isSimulated` exists specifically so the
  UI/report can label it transparently ("Simulated attachment — placeholder for demonstration")
  rather than presenting a fictional seed-data attachment as if it were genuine evidentiary content.
...
- **v1 implementation — `PostgresBlobStorageProvider`** (corrected per `technical-architecture.md`
  §9/TA-1, closing spec-review.md SR-001/SR-002 — supersedes this section's original
  `LocalDiskStorageProvider`/`DATA_DIR` design, which does not work on Vercel's ephemeral serverless
  filesystem): `save` generates an opaque `storagePath` handle; the caller persists the bytes in
  `Attachment.fileBytes` (§3.10) in the same operation that creates the row. `retrieve`/`delete`
  look the row up by `storagePath` and operate on `fileBytes` directly. This is the only
  implementation shipped in v1.
- **Future implementation — e.g. `ObjectStorageProvider`**: would implement the identical interface
  against S3-compatible or similar object storage, with `storagePath` becoming a real object key
  instead of an opaque database-only handle, and `fileBytes` no longer populated (the object store
  holds the content instead). Because `Evidence`/`Attachment` and every FR that touches them (FR-021–
  FR-024) only ever call the interface, swapping providers is a configuration change plus one new
  class — no change to validation rules, no change to any functional requirement's behavior.
  `storagePath`'s type (`VARCHAR(500)`, opaque) is chosen specifically to remain valid under either
  implementation.
