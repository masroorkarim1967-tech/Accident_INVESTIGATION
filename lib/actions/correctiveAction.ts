...
} from "@/lib/validation/correctiveAction";
import {
...
// FR-044/045a/045b: "the action's own owner" is a role-independent carve-out
// (any non-Viewer role may be set as an owner and act on their own action),
// so the outer gate here is broader than EDIT_ROLES — fine-grained
// eligibility is decided by lib/services/actionLifecycle.ts afterward.
const ACTOR_ROLES = [UserRole.Administrator, UserRole.InvestigationManager, UserRole.Investigator, UserRole.Reviewer];

export type CorrectiveActionState = { error: string | null; fieldErrors?: Record<string, string> };

function mapEditAccessError(error: unknown): CorrectiveActionState | null {
  if (error instanceof AuthorizationError) return { error: error.message };
  if (error instanceof NotFoundError) return { error: error.message };
  return null;
}

/** FR-040 — Add/Edit Corrective Action. actionId omitted (or null) means "add new". */
export async function saveCorrectiveActionAction(
  investigationId: number,
  actionId: number | null,
  _prevState: CorrectiveActionState,
  formData: FormData,
  ...
  // FR-040 edge case: the future-date restriction applies only to a new
  // action's initial Target Date, not to later edits.
  if (!actionId) {
  ...
  if (actionId) {
    const existing = await db.correctiveAction.findUniqueOrThrow({ where: { id: actionId } });
...
export async function removeCorrectiveActionAction(investigationId: number, actionId: number): Promise<CorrectiveActionState> {
  let user;
  try {
  ...
  const action = await db.correctiveAction.findUnique({ where: { id: actionId } });
...
export async function markCorrectiveActionCompleteAction(
  investigationId: number,
  actionId: number,
  _prevState: CorrectiveActionState,
  formData: FormData,
...
export async function verifyCorrectiveActionEffectivenessAction(
  investigationId: number,
  actionId: number,
  _prevState: CorrectiveActionState,
  formData: FormData,
...
export async function reassignCorrectiveActionOwnerAction(
  investigationId: number,
  actionId: number,
  _prevState: CorrectiveActionState,
  formData: FormData,
  ...
  const existing = await db.correctiveAction.findUnique({ where: { id: actionId } });
